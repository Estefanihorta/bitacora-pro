...
import AppA1Inner from './AppA1';
import AppA2Inner from './AppA2';
...
export default function App() {
  ...
  const MODS = [
    { id: "planner", icon: "📋", label: "Planner", desc: "Planificacion diaria", color: P.gold, app: "A1" },
    { id: "productos", icon: "📦", label: "Productos", desc: "Pipeline de productos", color: P.blue, app: "A1" },
    { id: "trafficker", icon: "📊", label: "Trafficker", desc: "Campanas Meta Ads", color: P.green, app: "A2" },
    { id: "creativos", icon: "🎨", label: "Creativos", desc: "Prompts y guiones", color: P.purple, app: "A2" },
    { id: "metricas", icon: "📈", label: "Metricas", desc: "Salud del negocio", color: P.orange, app: "A2" },
  ];
  ...
  if (screen === "loading") return (
    <div style={{ background: P.bg, minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      <style>{CSS}</style>
      <div style={{ fontFamily: "'Poppins',sans-serif", fontSize: 32, fontWeight: 800, lineHeight: 1 }}><GT>BITACORA PRO</GT></div>
      <div style={{ fontSize: 9, color: P.mt2, letterSpacing: 4, textTransform: "uppercase", marginTop: 6, fontWeight: 300 }}>by Estefani Horta</div>
      <div style={{ display: "flex", gap: 6, marginTop: 12 }}>{[0, 1, 2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: P.gold, animation: "pulse 1.4s ease-in-out " + (i * 0.2) + "s infinite" }} />)}</div>
    </div>
  );
  ...
  // Decide which inner app to show based on module
  const currentMod = MODS.find(m => m.id === modulo);
  const showA2 = currentMod && currentMod.app === "A2";
  ...
  return (
    <div style={{ fontFamily: "'Poppins',sans-serif", background: P.bg, minHeight: "100vh", color: P.tx }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 185 }} onClick={() => setMenuOpen(false)}>
          ...
          <div style={{ position: "absolute", top: 0, right: 0, width: isMob ? "85%" : "290px", height: "100vh", background: "#080806", borderLeft: "1px solid " + P.border, display: "flex", flexDirection: "column" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 12, borderTop: "1px solid " + P.border, display: "flex", gap: 8 }}>
              <button style={{ ...SB({ padding: "9px 0", fontSize: 11 }), flex: 1, display: "block", textAlign: "center" }} onClick={() => setMenuOpen(false)}>Cerrar menu</button>
      ...
      {/* Modules - delegate to sub-apps */}
      {modulo !== "home" && !showA2 && <AppA1Inner initialModulo={modulo} user={user} onLogout={logout} />}
      {modulo !== "home" && showA2 && <AppA2Inner initialModulo={modulo} user={user} onLogout={logout} />}
    </div>
  );
}
