...
/* ── DETECTAR SI LOS DATOS GUARDADOS SON VIEJOS ── */
...
export default function AppA2({embeddedUser=null,embeddedModulo=null,onLogout=null}){
  ...
  const[selProd,setSelProd]=useState("Todos");
    ...
    <div style={{fontFamily:"'Poppins',sans-serif",background:P.bg,minHeight:"100vh",color:P.tx}}>
      ...
      {modulo==="home"&&(
        <div style={{padding:isMob?"14px":"26px 30px",maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:isMob?"1fr":"repeat(3,1fr)",gap:11,marginBottom:18}}>
          ...
          {(()=>{const r=calcCosteo(calcV);const F=COP;
            ...
            return(<>
              ...
              <div style={{background:P.bg2,borderRadius:8,padding:"8px 10px",border:"1px solid "+P.border,marginBottom:6}}>
                {[["Proveedor",r.prec],["Flete",r.fleteD],["CPA",r.cpa],["Total",r.total]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid "+P.border}}><span style={{fontSize:10,color:P.mt}}>{l}</span><span style={{fontSize:10,color:l==="Total"?P.gold:P.mt,fontWeight:l==="Total"?700:400}}>{F(v)}</span></div>))}
              </div>
              ...
              <div style={{background:P.bg2,borderRadius:8,padding:"8px 10px",border:"1px solid "+P.border}}>
                {[["x2 (-20%)",r.pv2,r.util2],["x3 (-30%)",r.pv3,r.util3]].map(([l,pv,ut])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid "+P.border}}><span style={{fontSize:10,color:P.mt}}>{l} {F(pv)}</span><span style={{fontSize:10,color:ut>=0?P.green:P.red,fontWeight:600}}>{F(ut)}</span></div>))}
              </div>
            </>);
          })()}
        </div>)}
